...
void input_schedule(int login_num)
{
	...
	printf("> 예약 관련 내용 입력(한글 최대 100자) : "); OUT;
	my_gets(newSchedule.note, sizeof(newSchedule.note));
...
void delete_schedule(int login_num)
{
	...
	int cnt;                 			// 예약 스케줄 수
	int index_ary[24][2];  	// 검색한 예약 스케줄의 예약자 및 위치 저장 (1일 예약 수는 최대 24개로 하며 별도 예외 처리 없음)
	...
	{
		if ( (s_ary[i].date.day == dDelete.day) &&	(s_ary[i].date.month == dDelete.month)
				&& (s_ary[i].date.year == dDelete.year))
		{
...
int on_schedule(int year, int month, int day)
{
	...
	for (i = 0; i < s_cnt; i++)
	{
		if ((s_ary[i].date.day == day) &&
				(s_ary[i].date.month == month) && (s_ary[i].date.year == year))
		{
			cnt++;
		}
	}
...
void print_schedule(DateT dPrint)
{
	...
	{
		if ((s_ary[i].date.day == dPrint.day) && (s_ary[i].date.month == dPrint.month) && (s_ary[i].date.year == dPrint.year))
		{
