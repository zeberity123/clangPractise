...
void enroll_num(void)
{
	...
	index = is_enroll_num(num);     // 등록이 되어 있는지 확인
...
void login_out(void)
{
	...
	printf("> 사번 : "); OUT;
	scanf("%d", &num);
	index = is_enroll_num(num);            // 등록된 사번이면 배열의 등록 위치 반환, 아니면 -1 반환
...
void input_schedule(void)
{
	...
	disp_calendar(year, month);        // 입력한 달의 달력 출력
...
int on_schedule(int year, int month, int day)
{
	...
	for (i = 0; i < s_cnt; i++)
	{
		if ( (s_day[i][3] == day) && (s_day[i][2] == month) && (s_day[i][1] == year))
		{
			cnt++;
		}
	}
...
void search_schedule(void)
{
	...
	printf("> 년, 월 입력 : "); OUT;
	scanf("%d%d", &year, &month);
	disp_calendar(year, month);            // 현재 달 출력, 선택한 날짜 반환

	printf("# 날짜를 선택하세요 : "); OUT;
	scanf("%d", &day);

	cnt = on_schedule(year, month, day);         // 선택한 날짜의 스케줄 수 반환
