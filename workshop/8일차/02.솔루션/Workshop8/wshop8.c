...
void search_schedule(void)
{
	...
	cnt = searchNodes(SAry, Head, dSearch);   // 선택한 날짜의 스케줄 수 반환
...
void delete_schedule(int login_num)
{
   ...
   char *errmsg[] = {
	...
	cnt = searchNodes(SAry, Head, dDelete); //on_schedule(dDelete.year, dDelete.month, dDelete.day);         // 선택한 날짜의 스케줄 수 반환
	if(cnt == 0)
		ERR_ROUTINE(1);
