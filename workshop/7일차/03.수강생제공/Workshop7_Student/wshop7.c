...
int main(void)
{
	...
	// todo : 파일에서 예약 스케줄 데이터 읽어 s_ary 배열에 저장하기
...
void input_schedule(int login_num)
{
	...
	// todo: newSchedule.note에 한글 최대 400자까지의 참고 내용을 입력 받아 저장한다.
	// 이때, get_str() 함수를 사용한다.
...
char *get_str(void)
{
	char str[801];   // 입력을 위해 임시로 사용되는 배열
	char *tp;   	    // 메모리 동적 할당을 받은 뒤 str의 내용을 복사하여 반환되는 포인터 변수

	// todo: 입력한 문자열을 동적할당 영역에 저장 후 포인터 반환한다.

	return tp;
}


// 프로그램 종료 - 스케줄 데이터를 파일에 저장하고 동적 할당 영역 반환
void exit_routine(void)
{

	int i;
	// todo: 예약 스케줄 데이터를 파일에 저장하고, s_ary 배열에서 사용된 동적 할당 영역을 반환한다.
...
void readdataFromFile(void)
{
	...
	// todo : schedule.txt 파일에서 예약 스케줄 데이터를 읽어 s_ary 배열에 저장한다.
...
void writedataToFile(void)
{
	...
	// todo : 예약 스케줄 데이터를 schedule.txt에 저장한다.
